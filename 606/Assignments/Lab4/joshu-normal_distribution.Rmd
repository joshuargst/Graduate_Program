...
The two distributions are generally normal however the women's height does have a right skeweness not exhibited in the men's height distribution. Men are also generally taller, what looks to be the bin wher the median lies ~180 for men, looks to be the maximum bin for women.
...
based on this plot, it does appear that the data follows a nearly normal distribution since the histogram generally follows that normal curve.
...
4.  Does the normal probability plot for `fdims$hgt` look similar to the plots 
    ...
    The normal probability plot for `fdims$hgt` do look similar to the plots created for the simulated data and thus provides evident that the female heights are nearly normal
...
The weights also seem to follow a normal distribution. The very first QQ plot does imply otherwise especially on the far ends of the distributions (z >2).
...
I would like to answer what percentage of fales weigh more than 70kg, and what percentage of men weigh more than 90 kg
```{R, eval = TRUE}
...
Based on the Normal plot showing the points deviating from the distribution on the right end of the of the normal plot, this is a right skewed distribution. this is confirmed in the histogram below.
