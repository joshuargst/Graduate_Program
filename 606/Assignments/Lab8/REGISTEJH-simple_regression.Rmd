...
The type of plot I would use is a scatter plot to determine if there is any relationship between two numerical variables. The relationship does look linear and i would consider using a linear model to predict the number of runs.
...
The relationship is a positive linear relationship with a pearson's r of 0.61. there are seemingly a few outliers and the spread of the data seems to increase with increased number of at_bats
...
```{r, eval=TRUE}
# Note that this chunk will only run in interactive mode
plot_ss(x = mlb11$at_bats, y = mlb11$hits)
plot_ss(x = mlb11$at_bats, y = mlb11$strikeouts)
plot_ss(x = mlb11$at_bats, y = mlb11$wins)
...
6.  Is there any apparent pattern in the residuals plot? What does this indicate
  ...
  there does not seem to be no apparent pattern, to me this implies that the relationship is not skewed by any particular points
...
```{r}
paste0("The R^2 of at_bats and runs was ", round(cor(mlb11$at_bats,mlb11$runs)^2,2))
paste0("The R^2 of hits and runs was ", round(cor(mlb11$hits,mlb11$runs)^2,2))
```
Based on the computation above, hits is a better predictor than at_bats using linear modelling.
...
```{r}
paste0("The R^2 of at_bats and runs was ", round(cor(mlb11$at_bats,mlb11$runs)^2,2))
paste0("The R^2 of homeruns and runs was ", round(cor(mlb11$homeruns,mlb11$runs)^2,2))
paste0("The R^2 of strikeouts and runs was ", round(cor(mlb11$strikeouts,mlb11$runs)^2,2))
paste0("The R^2 of wins and runs was ", round(cor(mlb11$wins,mlb11$runs)^2,2))
paste0("The R^2 of stolen bases and runs was ", round(cor(mlb11$stolen_bases,mlb11$runs)^2,2))
```

```{r}
plot(mlb11$strikeouts,mlb11$runs)
m2<-lm(mlb11$runs~mlb11$strikeouts)
summary(m2)
...
mlb11$predicted<-predict(m2)
mlb11$residual<-residuals(m2)

ggplot(mlb11,mapping = aes(x = strikeouts))+
  geom_point(aes(y = runs))+
  geom_line(aes(y = predicted),size= 1.5, alpha = .2)+
  geom_segment(aes(xend = strikeouts,yend = predicted,y = runs),linetype = 2, color = 'blue3')+
  theme_bw()
...
```{r}
paste0("The R^2 of new_slug and runs was ", round(cor(mlb11$new_slug,mlb11$runs)^2,2))
paste0("The R^2 of new_onbase and runs was ", round(cor(mlb11$new_onbase,mlb11$runs)^2,2))
paste0("The R^2 of new_obs and runs was ", round(cor(mlb11$new_obs,mlb11$runs)^2,2))
...
m2<-lm(mlb11$runs~mlb11$new_obs)
summary(m2)
...
ggplot(mlb11,mapping = aes(x = new_obs))+
  geom_point(aes(y = runs))+
  geom_line(aes(y = predicted),size= 1.5, alpha = .2)+
  geom_segment(aes(xend = new_obs,yend = predicted,y = runs),linetype = 2, color = 'blue3')+
  theme_bw()

```

The new observations turned out to be a better predictor based on the much higher r^2 value and the reliable spread around the regression line.
