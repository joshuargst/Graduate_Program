...
This is likely a sample statistic based on global polling. The intent is likely to capture majority of the population with the poll and use that sample to estimate the population distribution.    
...
We must assume that this is a random sample for independence and sample size is large enough to estimate population.
...
Each row in table 6 corresponds to the country, sample size, % religious, % non-religious, %,atheist, % unknown

Each row in in atheism dataframe correspond to state, response (atheist, religious, unknown, etc) and year
...
,1),"%")
```
The number listed in the table is about 5.3% which is what is shown in Table 6 when rounded.
...
Conditions for inference are that it must be a random sample which it is. and that the population proportion n*p and n*(1-P) must be greater than 10. All the conditions are met
...
The marjin of error for a 95% confidence interval based on the R output is [3.64% to 6.34%]
...
as the population proportion deviates from 0.5, the margin of error decreases in what looks like a polynomial-like function.
...
the sample distribution of p = 1 and n = 1040 has mean around 0.1, takes on a bell shaped normal distribution curve and a spread that goes to about .065 to .125
...
hist(p_hats, main = paste0("p = ",p,", n = ",n), xlim = c(.1, 0.3))
...
adjusting the proportion higher seems to increase the spread of the data but increasing the center. adjusting the number of samples lower also seems to increase the spread and flatten out peak.
...
I believe it is sensible to report margin of errors because these distributions can clearly be affected by the number of samples and sampling proportions.
...
```{r}
us05 <- subset(atheism, nationality == "Spain" & year == "2005")
us12 <- subset(atheism, nationality == "Spain" & year == "2012")
...
```
 ...
 forming confidence intervals for both spain and US in 2005 and 2012 show that there is no significant difference between the years as there is significant overlap between the intervals with 95% confidence.
...
we would not expect to detect a change of more than 5% of countries if the null hypothesis (no change) is true.
...
margin of error on plot >.1 corresponds to a population proportion of about 0.01 and to obtain np = 10, we would need 10/.01 or a sample count of 1000.
