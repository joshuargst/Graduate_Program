...
The population distribution skewed right
...
The distribution of the sample is skewed right similar to the population, however the modes are lower and the peaks less pronounced relative to the surrounding values.
...
The mean of samp2 is different from that of samp1 but it similarly is not equal to the population mean. if we took larger samples, we would expect this to be closer to the population mean.
...
there are 5000 means in the sample_means50 and the sampling distribution follows the gaussian distribution. i would expect the distribution to remain normal when collecting more samples, but the spread to be decreased.
...
```
There are 100 elements and each element represents a mean from a sample of 50 taken from a population of 2930.
...
when the sample size is larger the center is taller and the spread is narrower.
...
paste("The mean price of our sample distribution is ",mean(samp1))
...
paste("The population mean is ", mean(price))
```
Based on our histogram, our sample mean seems to lie just under 180000.
...
```
    Based on this histogram, the spread of our data is smaller and our peak mode is more pronounced and similarly our sample mean lies around 180.
...
The 3rd sampling distribution has a smaller spread and if we're concerned with making estimates closer to the true value, we would prefer a distribution with a smaller spread.
