...
The distribution of the sample is normal where the typical size is between 500 to 2000 seemingly containing most of the points, and our mode is between 1000 and 1500.

2.  Would you expect another student's distribution to be identical to yours? 
Would you expect it to be similar? Why or why not?

I would not expect another student's distribution to be identical, but i would expect this to be similar. This is because they will be pulling from the same population but not exactly the same samples.
...
for this condition to be true the sample must be independent and this is acheived via random sampling. 

## Confidence levels

4.  What does "95% confidence" mean? If you're not sure, see Section 4.2.2.

95% confidence interval means that for a normal distribution of means, we can expect the true population mean to be within -1.96< z < 1.96 deviations of what the sample mean distribution says it is.
...
yes, our confidence interval does capture the true mean.
...
I would expect 95% of the students to capture the true population mean. this is because if we take 100 samples and are using a 95% confidence interval threshold, we can expect 95 of those samples to have the true population mean.
...
-   Using the following function (which was downloaded with the data set), 
    ...
    ```
This proportion is not exactly equal to the confidence interval. 3/50 samples are outside the population mean and this is 6% of the samples. This is due to a combination of random chance and the fact that just 50 samples were collected so we cannot get excatly 5% error.
...
#10%
conf = .10
(Critical_value <- qnorm(1-(conf/2)))
```
If i chose a confidence interval of 10%, our critival value would then be 1.64 and we would use this to establish our upper and lower limits rather than 1.96 for a 5% conf interval.
...
plot_ci(lower_vector, upper_vector, mean(population))
```
with this, we can see that we have 5/50 samples that do not contain the actual population mean and this reflects our 90% confidence interval as 10% of the samples were not within range.
