...
The command I would use to extract just the counts of girls baptized is similar to the command above as such:
...
Yes there is an apparanent trend in the number of girls baptized over the years. The trend shows number of baptized girls drops significantly between 1640 and 1660 but the general direction is an increase in baptisms over time.
...
```{r plot-of-proportion-of-boys, eval = TRUE}
plot(arbuthnot$year,arbuthnot$boys / (arbuthnot$boys + arbuthnot$girls), type = 'l')
```
One thing that I see clearly in this plot is that the proportion of boys that are baptised is always greater than the proportion of girls baptised, but not by a large margin.
...
```{r Years-and-dimension, eval = TRUE}
...
We can see that this data set includes years from 1940 to 2002. The dimensions of this dataset are 63 x 3, or 63 observations of 3 variables. the variables being observed are also in the column names and are "year", "boys", and "girls".



-   How do these counts compare to Arbuthnot's? Are they on a similar scale?

```{r presentdataset, eval = TRUE}
present
```
These counts are not on a similar scale. they are about 3 orders of magnitude greater than the counts from the arbuthnot dataset.
...
```{r plotboytogirl-ratio, eval = TRUE}
plot(present$year, present$boys/present$girls, type = 'l')

```

Similar to the arbuthnot dataset, The ratio of boys to girls is always >1 and so the same observations hold true in the U.S. and for that this particular time period as shown in the plot.
...
present$year[
  match(
      max(present$boys+present$girls),(present$boys+present$girls)
      )
  ]
```
Using some tools from the R reference card file, I am able to find the index in which the maximum number of children were baptized and reference that index to the year column in the present table. This year is 1961.
