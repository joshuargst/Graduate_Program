...
class PandasChain:
    ...
    def __commit_block(self,block): 
        ...
        # Add code here
        block_hash.update(str(self.seq_id).encode('utf-8'))
        block_hash.update(str(self.prev_hash).encode('utf-8'))
        block_hash.update(str(self.transactions).encode('utf-8'))
...
class Block:
    ...
    def display_header(self): 
        return self.__seq_id, self.__status, self.__block_hash, self.__merkle_tx_hash, self.display_transactions
    
    # 10 pts - This is the interface for how transactions are added
    def add_transaction(self,s,r,v): 
        ts = str(dt.datetime.now()).encode('utf-8') # Get current timestamp 
        tx_hash = hashlib.sha256(ts).hexdigest()# Hash of timestamp, sender, receiver, value
        new_transaction = pd.DataFrame({'Timestamp':[ts],
                                        'Sender':[s],
                                        'Receiver':[r],
                                        'Value':[v],
                                        'TxHash':[tx_hash]}) # Create DataFrame with transaction data (a DataFrame with only 1 row)
        self.__transactions.append(new_transaction) # Append to the transactions data
    ...
    def get_simple_merkle_root(self): 
        smr = ''.join(self.__transactions['TxHash'])
        smrstr = str(smr).encode('utf-8')
        merkleroot = hashlib.sha256(smrstr).hexdigest()
        return merkleroot
