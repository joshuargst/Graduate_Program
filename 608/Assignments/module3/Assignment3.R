...
{ui <- 
  ...
  server <- function(input, output) {
    ...
    output$Question2_PlotA<-renderPlotly({
      ...
      Allannot<-mean(filter(Mortalitydata,Year==2010)$Crude.Rate)
      Subannot<-mean(filter(Mortalitydata_sub,Year==2010)$Crude.Rate)
      ...
      ggplotly(
        ggplot()+
          ...
          annotate(geom = "text", x = 2010.5, y =Subannot, label =input$RateofDeath)
      )
    })
